...
func (cm *cacheManager) queryListObject(req *http.Request) (runtime.Object, error) {
	...
	if err == storage.ErrStorageNotFound && proxyutil.IsListRequestWithNameFieldSelector(req) {
		...
		// when the specified resource is not found, to keep same as APIServer.
		return listObj, nil
	}
	if err != nil {
...
func (cm *cacheManager) saveOneObject(ctx context.Context, info *apirequest.RequestInfo, b []byte) error {
	...
	if err := cm.storeObjectWithKey(key, obj); err != nil {
		return err
	}
...
func (cm *cacheManager) storeObjectWithKey(key storage.Key, obj runtime.Object) error {
	...
	case storage.ErrStorageNotFound:
		...
		if err = cm.storage.Create(key, obj); err == storage.ErrStorageAccessConflict {
