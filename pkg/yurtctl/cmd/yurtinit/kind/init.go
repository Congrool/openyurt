...
func NewKindInitCMD() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "kind",
		Short: "Setup OpenYurt cluster with kind",
...
func (o *kindOptions) Config() *initializerConfig {
	controlPlaneNode, workerNodes := getNodeNamesOfKindCluster(o.ClusterName, o.NodesNum)
...
func addFlags(flagset *pflag.FlagSet, o *kindOptions) {
	flagset.StringVar(&o.KindConfigPath, "kind-config-path", o.KindConfigPath,
		"Specify the customized kind config path.")
	flagset.IntVar(&o.NodesNum, "nodes-num", o.NodesNum,
	...
	flagset.BoolVar(&o.UseLocalImage, "use-local-image", o.UseLocalImage,
		"If set, local images stored by docker will be used first. The default is false.")
...
func (i *Initializer) prepareKindConfigFile(kindConfigPath string) error {
	paths := strings.Split(kindConfigPath, "/")
	kindConfigDir := strings.Join(paths[:len(paths)-1], "/")
...
func getGoBinPath() (string, error) {
	...
	return strings.Join([]string{string(gopath), "bin"}, "/"), nil
}
